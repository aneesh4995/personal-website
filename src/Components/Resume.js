  ...
  render() {
    ...
    if(this.props.data){
      ...
      var education = this.props.data.education.map(function(education){
        ...
        <p>{education.course}</p></div>
      })
      var work = this.props.data.work.map(function(work){
        return <div key={work.company}><h3>{work.company}</h3>
            <p className="info">{work.title}<span>&bull;</span> <em className="date">{work.years}</em></p>
            <p className="newline">{work.description1}</p>
            <p className="newline">{work.description2}</p>
            <p className="newline">{work.description3}</p>
            <p className="newline">{work.description4}</p>
            <p className="newline">{work.description5}</p>
        </div>
      })
      ...
      var databases = this.props.data.skills.databases.map(function(skills){
      var projectImage = 'images/tech/'+skills.image;
        return <div key={skills.name} className="columns feature-item">
                  <img className='skill' alt={skills.name} src={projectImage} />
                  <h5>{skills.name}</h5>
      ...
      var devops = this.props.data.skills.devops.map(function(skills){
      var projectImage = 'images/tech/'+skills.image;
        return <div key={skills.name} className="columns feature-item">
                  <img className='skill' alt={skills.name} src={projectImage} />
                  <h5>{skills.name}</h5>
